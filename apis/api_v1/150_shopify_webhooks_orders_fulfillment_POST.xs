query "shopify/webhooks/orders_fulfillment" verb=POST {
  api_group = "api_v1"
  ...
  stack {
    ...
    conditional {
      if ($fulfillment_status == "fulfilled") {
        // FULFILLMENT LOGIC
        // Ensure order is in a valid state for fulfillment (Allocated/In Progress)
        conditional {
          if ($order_record.status_internal != "allocated" && $order_record.status_internal != "partially_allocated" && $order_record.status_internal != "in_progress") {
            throw {
              name = "PreconditionFailed"
              value = "Order status must be IN_PROGRESS (allocated) to fulfill. Current status: " ~ $order_record.status_internal
            }
          }
        }
      
        var $processed_allocations {
          value = []
        }
      
        // Load order lines for this order
        db.query order_line {
          where = $db.order_line.order_id == $order_record.id
          return = {type: "list"}
        } as $order_lines
        ...
        foreach ($order_lines) {
          each as $line {
            ...
            db.query order_component_allocation {
              where = $db.order_component_allocation.order_line_id == $line.id
              return = {type: "list"}
            } as $allocations
          
            foreach ($allocations) {
              each as $allocation {
                var $quantity_to_consume {
                  value = $allocation.quantity_allocated - $allocation.quantity_consumed
                }
                ...
                conditional {
                  if ($quantity_to_consume > 0) {
                    ...
                    db.query inventory_balance {
                      where = $db.inventory_balance.tenant_id == $tenant_id && $db.inventory_balance.component_id == $allocation.component_id && $db.inventory_balance.location_id == $allocation.location_id
                      return = {type: "single"}
                    } as $balance
                    ...
                    conditional {
                      if ($balance != null) {
                        ...
                        db.edit inventory_balance {
                          ...
                          data = {
                            in_progress_qty: $balance.in_progress_qty - $quantity_to_consume
                            shipped_qty    : $balance.shipped_qty + $quantity_to_consume
                            updated_at     : "now"
                          }
                        } as $updated_balance
                        ...
                        db.edit order_component_allocation {
                          ...
                          data = {
                            quantity_consumed: $allocation.quantity_consumed + $quantity_to_consume
                            updated_at       : "now"
                          }
                        } as $updated_allocation
                        ...
                        db.add inventory_movement {
                          data = {
                            ...
                            quantity_delta    : 0 - $quantity_to_consume
                            quantity_after    : $updated_balance.on_hand_qty
                            reference_type    : "ORDER"
                            reference_id      : $order_record.id
                            reason_code       : "shopify_fullfilment"
                            note              : ["Fullfiled by Shopify Order",$var.order_raw.order_number]|join:" "
                            created_by_user_id: null
                          }
                        }
                      
                        array.push $processed_allocations {
                          value = $updated_allocation
                        }
                      }
                    }
                  }
        ...
        db.edit order {
          ...
          data = {status_internal: "FULFILLED", updated_at: "now"}
        } as $final_order
